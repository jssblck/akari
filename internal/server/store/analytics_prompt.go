...
// PromptHygiene aggregates the scoped sessions' prompt-hygiene counts for the Insights
...
// row, so the sums cover exactly the sessions whose hygiene has been measured at the
// running model. That measured cohort (Sessions) is the base for the unstructured-start
// rate; it can be smaller than the page's total session count only while a backfill
// reparse is mid-flight, and the parsed views are gated during a reparse, so a reader
// never sees the two disagree. The prompt denominator is the stored prompt_count (the
// classifier's own base of non-empty prompts), not user_message_count, so a numerator can
// never exceed it and every rate stays within [0, 1].
	...
	err := s.Pool.QueryRow(ctx,
		`SELECT coalesce(sum(sig.prompt_count), 0),
		   ...
		   JOIN session_signals sig
		     ON sig.session_id = s.id AND sig.signals_version = $`+fmt.Sprint(len(args))+`
		  WHERE TRUE`+filter,
