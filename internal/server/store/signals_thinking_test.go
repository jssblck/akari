...
// TestObservedThinkingStats aggregates the fleet turn distribution: one session whose five
// assistant turns land one per band (off, low, medium, high, very high) contributes each turn to
// the split, and the token percentiles read over the four thinking turns. The unit is the turn,
// not the session, so the counts are turn counts.
func TestObservedThinkingStats(t *testing.T) {
	t.Parallel()
	st, _, uid, pid := signalsEnv(t)
	ctx := t.Context()

	// One session, five turns: off, then 100 (low), 300 (medium), 1000 (high), 3000 (very high)
	// tokens. The band edges are 128 / 512 / 2048, so those token counts land one per band.
	thinkingSession(t, st, uid, pid, "sess-agg", "claude-fable-5",
		[]int{0, bytesForTokens(100), bytesForTokens(300), bytesForTokens(1000), bytesForTokens(3000)})

	stats, err := st.ObservedThinking(ctx, store.AnalyticsFilter{})
	if err != nil {
		t.Fatalf("observed thinking stats: %v", err)
	}
	if stats.Turns != 5 {
		t.Fatalf("measured turns = %d, want 5", stats.Turns)
	}
	if stats.Off != 1 || stats.Low != 1 || stats.Medium != 1 || stats.High != 1 || stats.XHigh != 1 {
		t.Errorf("band split = off %d low %d medium %d high %d xhigh %d, want 1 each",
			stats.Off, stats.Low, stats.Medium, stats.High, stats.XHigh)
	}
	// Percentiles over the thinking-turn tokens [100, 300, 1000, 3000]. percentile_cont
	// interpolates, so p50 lands between 300 and 1000 at the midpoint (650); p90 and p99 sit high
	// in the [1000, 3000] gap. The interpolation is floating point, so assert within a token.
	approx := func(name string, got, want float64) {
		if got < want-1 || got > want+1 {
			t.Errorf("%s = %v, want ~%v", name, got, want)
		}
	}
	approx("p50", stats.P50, 650)
	approx("p90", stats.P90, 2400)
	approx("p99", stats.P99, 2940)
}

// TestObservedThinkingStatsEmpty pins the no-data guard: a scope with no measured turns reports
// HasData false rather than a row of zeros, so the panel shows its empty note.
func TestObservedThinkingStatsEmpty(t *testing.T) {
	t.Parallel()
	st, _, _, _ := signalsEnv(t)
	ctx := t.Context()
	...
	if stats.HasData() {
		t.Errorf("empty scope should report no data, got %d turns", stats.Turns)
	}
}
