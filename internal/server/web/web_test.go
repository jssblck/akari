...
func TestSessionPageShowsDuplicateIDChip(t *testing.T) {
	...
	html := renderComponent(t, SessionPage(p, d, msgs, tools, nil, nil, store.CacheStats{}, 1, false, false))
	if !strings.Contains(html, "1 duplicate id") {
		t.Error("session page should show the duplicate-id chip when the count is non-zero")
	}

	html = renderComponent(t, SessionPage(p, d, msgs, tools, nil, nil, store.CacheStats{}, 0, false, false))
